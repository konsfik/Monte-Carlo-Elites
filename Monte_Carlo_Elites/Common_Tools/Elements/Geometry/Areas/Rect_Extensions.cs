...
namespace Common_Tools.Elements
{
    public static class Rect_Extensions
    {
        ...
        public static HashSet<UEdge2i> Contained_Ortho_UEdges__Set(
            this Rect2i rect
            )
        {
            ...
            HashSet<UEdge2i> contained_u_edges = new HashSet<UEdge2i>();
            for (int x = min_x; x <= max_x; x++)
            {
                for (int y = min_y; y <= max_y; y++)
                {
                    ...
                    if (x < max_x)
                    {
                        ...
                        UEdge2i edge = new UEdge2i(origin, exit);
        ...
        public static List<UEdge2i> Contained_Ortho_UEdges__List(
            this Rect2i rect
            )
        {
            int min_x = rect.min_coords.x;
            ...
            List<UEdge2i> contained_u_edges = new List<UEdge2i>();
            for (int x = min_x; x <= max_x; x++)
            {
                for (int y = min_y; y <= max_y; y++)
                {
        ...
        {
            return
                point.x.Is_In_Range(rect.min_coords.x, rect.max_coords.x)
                &&
                point.y.Is_In_Range(rect.min_coords.y, rect.max_coords.y);
        }
